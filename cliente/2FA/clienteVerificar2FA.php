...
if (empty($_SESSION['id']) || $_SESSION['usuario'] !== 'cliente') {
...
if (empty($codigoIngresado) || empty($secretBase32)) {
    header("Location: clienteHabilitar2FA.php?error=faltan_datos");
    exit();
}
...
if (!$isValid) {
    header("Location: clienteHabilitar2FA.php?error=codigo_invalido");
    exit();
}

// 5) Guardar el secreto en la BD después de la verificación exitosa
$id_usuario = $_SESSION['id'];
$sqlUpdate = "UPDATE clientes SET google_2fa_secret = ? WHERE cliente_id = ?";
$stmt = mysqli_prepare($conn, $sqlUpdate);
mysqli_stmt_bind_param($stmt, "si", $secretBase32, $id_usuario);
mysqli_stmt_execute($stmt);

// Redirigir con éxito
header("Location: /cliente/index.php?mensaje=2fa_habilitado");
exit();
